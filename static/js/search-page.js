...
function renderSnippetList(snippets) {
    ...
    const visibleSnippets = snippets.slice(0, 2);
    return `
...
function renderMatchedItems(result) {
    ...
    return `
        <div class="search-item-list">
            ${previewItems.map((item) => `
                <button type="button" class="search-item-match" data-search-meeting-id="${escapeHtml(result.meeting_id)}" data-search-item-id="${escapeHtml(item.item_id || "")}" data-search-document-id="${escapeHtml(result.document_id || "")}">
                    <div class="search-item-top">
                        ...
                        <span>總分 ${escapeHtml(formatScore(item.final_score))}</span>
                    </div>
            ...
            `).join("")}
            ${hiddenCount ? `<div class="search-hidden-note">另有 ${hiddenCount} 筆命中項目，請點入會議預覽查看。</div>` : ""}
        </div>
    `;
}
...
function renderScoreDetail(detail, includeRecency = true) {
    ...
    const keys = includeRecency
        ? ["keyword_score", "structure_score", "task_score", "recency_score", "feedback_score"]
        : ["keyword_score", "structure_score", "task_score", "feedback_score"];
...
function renderRelatedMeetings(items) {
    if (!items.length) {
        elements.relatedMeetings.innerHTML = '<div class="related-empty">目前沒有可推薦的相關會議。</div>';
        return;
    }
...
function renderRelatedItems(items) {
    if (!items.length) {
        elements.relatedItems.innerHTML = '<div class="related-empty">目前沒有可推薦的相關項目。</div>';
        return;
    }
...
function renderSearchStats(data) {
    ...
    elements.statsPanel.innerHTML = `
        ...
        <details class="stats-collapse">
            ...
            ${renderCompactList(recentSearches, (item) => `${escapeHtml(item.query || "(空查詢)")} <span>${escapeHtml(String(item.result_count ?? 0))}</span>`)}
        </details>
    `;
}
