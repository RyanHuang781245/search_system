...
def build_extraction_prompt(text: str) -> str:
    """Build a meeting-minutes-focused extraction prompt."""
    return (
        "You are extracting a knowledge graph from meeting minutes and action-item lists.\n\n"
        "Text characteristics:\n"
        "- Each line may be an action item, discussion note, product note, or specification note.\n"
        "- Many lines may include an assignee and a date.\n"
        "- Preserve original Chinese and English product terms exactly as written.\n\n"
        "Extraction goals:\n"
        "1. Prefer ActionItem-centric extraction over generic entity extraction.\n"
        "2. If a line contains a task, request, owner, due date, product, topic, or region, extract them.\n"
        "3. Create multiple relationships from one line when needed.\n"
        "4. Extract only information stated in the text. Do not invent missing owners, dates, or products.\n\n"
        "Preferred relationship labels:\n"
        "- ASSIGNED_TO\n"
        "- DUE_ON\n"
        "- ABOUT\n"
        "- MENTIONS\n"
        ...
        "- TARGETS_REGION\n\n"
        "Normalization rules:\n"
        "- Represent each task as a concise ActionItem node, for example: Action:確認cemented type的Stem品號\n"
        "- Dates should remain in their original yyyy-mm-dd form when present.\n"
        "- If a line is not a task but mentions a product or spec, extract the clearest explicit relation.\n"
        "- Return relationships for every meaningful bullet or line, not just the most obvious one.\n\n"
        f"Meeting text:\n{text}"
    )
